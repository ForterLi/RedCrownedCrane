...
struct RCErrorHandler {

    static let shared = RCErrorHandler()
    ...
    func resultType(with error: Error?) -> CKOperationResultType {
        ...
        case .serviceUnavailable,
            ...
            if let retry = userInfo[CKErrorRetryAfterKey] as? Double {
                print("RCErrorHandler - \(message). Should retry in \(retry) seconds.")
        ...
        case .networkUnavailable,
             .networkFailure:
            print("RCErrorHandler.recoverableError: \(message)")
        ...
        case .partialFailure:
            ...
            if let dictionary = e.userInfo[CKPartialErrorsByItemIDKey] as? NSDictionary {
                print("RCErrorHandler.partialFailure for \(dictionary.count) items; CKPartialErrorsByItemIDKey: \(dictionary)")
            }
            return .recoverableError(reason: .partialFailure, message: message)
            
        // SHOULD CHUNK IT UP
        case .limitExceeded:
            print("RCErrorHandler.Chunk: \(message)")
            return .chunk
        ...
        case .alreadyShared,
             ...
             .referenceViolation,
             .tooManyParticipants:
            print("RCErrorHandler.Fail: \(message)")
